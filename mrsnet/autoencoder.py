...
class Autoencoder:
  ...
  def save(self, folder):
    ...
    if total_params is None:
      try:
        total_params = int(self.ae.count_params())
      except Exception:
        total_params = 0
    if trainable_params is None:
      try:
        trainable_params = int(np.sum([tf.size(v).numpy() for v in self.ae.trainable_variables]))
      except Exception:
        trainable_params = 0
    if non_trainable_params is None:
      try:
        non_trainable_params = int(np.sum([tf.size(v).numpy() for v in self.ae.non_trainable_variables]))
      except Exception:
