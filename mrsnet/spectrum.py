...
npfft = getattr(
    __import__(Cfg.val["npfft_module"][0], fromlist=[Cfg.val["npfft_module"][1]]),
...
class Spectrum:
    ...
    def __init__(
        self, id, pulse_sequence, acquisition, omega, source=None, metabolites=None, linewidth=None
    ):
    ...
    def set_f(
        ...
        b0_shift_ppm=0,
        scale=1.0,
        remove_water_peak=False,
        phase_correct=False,
        force_phase_correct=None,
    ):
    ...
    def _set(
        ...
        self.scale = scale
        if (phase_correct and Cfg.val["phase_correct"] is not None) or (
            force_phase_correct is not None
        ):
    ...
    def get_f(self):
        ...
        return (
            self.fft * self.scale,
            np.linspace(-1, 1, len(self.fft)) * self.sample_rate / 2.0 / self.omega
            + self.center_ppm
            + self.b0_shift_ppm,
        )
    ...
    def get_t(self):
        ...
        """
        return (
            npfft.ifft(npfft.ifftshift(self.fft * self.scale)),
            ...
            if abs(location - nu[idx]) < ppm_range:
                ...
                if Cfg.val["fft_peak_location_estimator"] == "quadratic":
                    ...
                    if np.abs(de) < 1e-10:
                        return nu[idx], -fft_abs[idx]  # zero denominator, return bucket freq.
                    ap = (
                        fft[idx + 1].real * fft[idx].real + fft[idx + 1].imag * fft[idx].imag
                    ) / de
                    dp = -ap / (1 - ap)
                    am = (
                        fft[idx - 1].real * fft[idx].real + fft[idx - 1].imag * fft[idx].imag
                    ...
                    f1 = np.sqrt(6.0) / 24.0
                    f2 = np.sqrt(2.0 / 3.0)
                    tau_dp2 = np.log(3.0 * (dp2**2) + 6.0 * dp2 + 1) / 4.0 - f1 * np.log(
                        (dp2 + 1.0 - f2) / (dp2 + 1.0 + f2)
                    )
                    tau_dm2 = np.log(3.0 * (dm2**2) + 6.0 * dm2 + 1) / 4.0 - f1 * np.log(
                        (dm2 + 1.0 - f2) / (dm2 + 1.0 + f2)
                    )
                    ...
                    raise RuntimeError(
                        f"Unknown fft peak location estimator {Cfg.val['fft_peak_location_estimator']}"
                    )
    ...
    def _phase_correct_ernst(self):
        ...
        def err(para):
            ...
            val = para.valuesdict()
            # Adjust phase
            shift = val["phi0"] + val["phi1"] * (
                np.linspace(-1, 1, len(self.fft)) * self.sample_rate / 2.0
            )
        ...
        if (
            max_s_fft * max_fft < 0.0
        ):  # avoid flipping the phase / turn real positive, if it should be negative
    ...
    def _phase_correct_acme(self):
        ...
        def entropy(para):
            ...
            h = r1 / np.sum(r1)
            h[np.abs(h) < 1e-8] = 1.0
            return -np.sum(h * np.log(h)) + Cfg.val["phase_correct_acme_gamma"] * np.sum(
                r[r < 0] ** 2
            )
    ...
    def rescale_fft(self, high_ppm=-4.5, low_ppm=-1, npts=2048):
        ...
        if (np.max(nu) < low_ppm) or (np.min(nu) > high_ppm):
            raise RuntimeError(
                f"Requested ppm rescale range out of range [{nu[0]},{nu[len(nu)-1]}]"
            )
    ...
    def _fft_remove_water_peak(self):
        ...
        water_peak_loc, _ = self._fft_peak_location(
            molecules.WATER_REFERENCE, Cfg.val["water_peak_ppm_range"]
        )
        if water_peak_loc is not None:
            ...
            for jj in range(0, len(abs_fft)):
                if (
                    nu[jj] >= water_peak_loc
                    and nu[jj] < water_peak_loc + Cfg.val["water_peak_ppm_range"] / 2.0
                ):
    ...
    def add_noise_adc_normal(self, mu=0, sigma=0):
        ...
        m = np.max(np.abs(adc))
        noise = (
            (np.random.normal(mu, sigma, len(adc)) + 1j * np.random.normal(mu, sigma, len(adc)))
            * m
            / self.scale
        )
    ...
    def plot(self, axes, type="fft", mode="magnitude"):
        ...
        if type == "time":
            ...
            axes.xaxis.set_major_formatter(
                FuncFormatter(lambda x_val, tick_pos: f"{np.abs(x_val):.8g}")
            )
        else:
    ...
    def plot_full_spectrum(spectra, concentrations={}, screen_dpi=96, type="fft"):
        ...
        metabolites = spectra[
            next(iter(spectra))
        ].metabolites  # Metabolites have to be the same for all spectra
        ...
        super_title += (
            next(iter(source))
            + " "
            + next(iter(pulse_sequence)).upper()
            + " "
            + f" @ {omega:.2f} MHz"
        )
    ...
    def comb(f1, s1, f2, s2, id, acq):
        ...
        s.set_f(
            f1 * fft1 + f2 * fft2,
            s1.sample_rate,
            center_ppm=s1.center_ppm,
            b0_shift_ppm=s1.b0_shift_ppm,
        )
        s.noise = s1.noise
        return s
    ...
    def combs(fs, ss, id, acq, allow_mixed_linewidths=False):
        ...
        for n in range(len(ss)):
            ...
            if not allow_mixed_linewidths:
                if (
                    (avg_linewidth is None and ss[n].linewidth is not None)
                    or (avg_linewidth is not None and ss[n].linewidth is None)
                    or (
                        avg_linewidth is not None
                        and np.abs(ss[n].linewidth - avg_linewidth) >= 1e-8
                    )
                ):
                    raise RuntimeError("Combing spectra with different linewidths")
        ...
        for n in range(1, len(ss)):
            ...
            fft += fs[n] * F
        s.set_f(
            fft, ss[0].sample_rate, center_ppm=ss[0].center_ppm, b0_shift_ppm=ss[0].b0_shift_ppm
        )
        s.noise = ss[0].noise
        return s
    ...
    def load_fida(fida_file, id, source, su=False):
        ...
        if su:
            ...
            s.set_t(
                np.conjugate(np.array(fida_data["fid"]).flatten()) / scale,
            ...
            # Conjugation due to negative frequencies
            s.set_f(
                np.conjugate(np.array(fida_data["fft"]).flatten()),
    ...
    def load_pygamma(
        ...
        with open(filename, "rb") as load_file:
            for raw in json.load(load_file):
                ...
                # Conjugation due to negative frequencies
                s.set_t(
                    np.array(raw["adc_re"]) - 1j * np.array(raw["adc_im"]),
                    1 / dt,
                    force_phase_correct=force_phase_correct,
                )
                specs.append(s)
            ...
            while parse:
                ...
                if line[0] == "$":
                    if area == "SEQPAR" or area == "BASIS" or area == "BASIS1" or area == "NMUSED":
                        ...
                        if np.abs(metadata["SEQPAR"]["HZPPPM"] - req_omega) > (
                            molecules.GYROMAGNETIC_RATIO / 5
                        ):
                            # more than a 0.2T difference, there's an issue
                            raise RuntimeError(
                                "LCModel basis set (%.2fT) is more than 0.2T different to prescibed "  # noqa: UP031
                                "omega (%.2fT)."
                                % (
                                    metadata["SEQPAR"]["HZPPPM"] / molecules.GYROMAGNETIC_RATIO,
                                    req_omega / molecules.GYROMAGNETIC_RATIO,
                                )
                            )
                        if metadata["SEQPAR"]["SEQ"] != "MEGA-":  # not megapress
                            raise RuntimeError(
                                "Unrecognised LCM pulse sequence: " + metadata["SEQ"]
                            )
                        try:
                        ...
                        if metabolite in req_metabolites:
                            ...
                            if len(nums) % 2 != 0:
                                raise RuntimeError(
                                    "Uneven fft number, the real/imag switching does not work here or the file has been loaded in wrong!"
                                )
                            fft = []
                            ...
                            fft_scale = (
                                metadata["BASIS"]["TRAMP"]
                                * 100.0
                                / (metadata["BASIS"]["CONC"] * metadata["BASIS"]["VOLUME"])
                            )
                            ...
                            if "PPMSEP" in metadata["NMUSED"]:
                                ...
                                center_ppm = -4.65
                            s = Spectrum(
                                id=os.path.basename(basis_file),
                                pulse_sequence="megapress",
                            ...
                            s.set_f(
                                npfft.fftshift(fft * fft_scale),
                                1.0 / metadata["BASIS1"]["BADELT"],
    ...
    def load_dicom(file, concentrations=None, metabolites=None, verbose=0):
        ...
        if pulse_sequence == "megapress":
            if "EDIT_OFF" in file:
                ...
                raise RuntimeError(
                    "Loaded dicom file for MEGA-PRESS, but acquisition cannot be determined.\n"
        ...
        if concentrations is not None:
            ...
            with open(concentrations) as f:
                js = json.load(f)
                if metabolites is None or len(metabolites) == 0:
                    metabolites = set()
                    for k in js.keys():
                        for l in js[k].keys():
                            metabolites.add(l)
        ...
        if Cfg.val["filter_dicom"] is not None:
            ...
            if Cfg.val["filter_dicom"] == "hamming":
                ...
                filterv[0 : filter_length // 2] = np.kaiser(
                    filter_length, Cfg.val["filter_dicom_kaiser"]
                )[filter_length // 2 :]
            else:
    ...
    def load_csv(file, concentrations=None, metabolites=None, verbose=0):
        ...
        ver = 0
        data = data_re = data_im = None
        sample_len = None
        time = None
        with open(file) as csvfile:
            rows = csv.reader(csvfile)
            for row in rows:
                if row[0][:15] == "WriteSpec2asc-v":
                    ver = int(row[0][15:])
                elif row[0] == "ID":
                    pid = row[1]
                    ...
                    if pulse_sequence in ["EJA_MPRESS"]:
                        ...
                        raise RuntimeError(
                            f"{file} - Unrecognised dicom pulse sequence: {pulse_sequence}"
                        )
                elif row[0] == "Transmitter_frequency":
                    omega = float(row[1]) / 1000000.0
                elif row[0] == "Sampling_frequency":
                    dt = 1.0 / float(row[1])
                elif row[0] == "FID":
                    data = np.array(row[1:], dtype=np.float64)
                elif row[0] == "FID_real":
                    data_re = np.array(row[1:], dtype=np.float64)
                elif row[0] == "FID_imag":
                    data_im = np.array(row[1:], dtype=np.float64)
                elif row[0] == "t":
                    time = np.array(row[1:], dtype=np.float64)
                elif row[0] == "TE":
                    pass
                elif row[0] == "Samples":
        ...
        # Combine FID data depending on version
        if ver == 1:
            if data_re is not None or data_im is not None:
                raise RuntimeError(
                    "FID_real and FID_imag are not expected to be present for version 1"
                )
            if data is None:
                raise RuntimeError("FID is expected to be present for version 1")
        elif ver == 2:
            if data_re is None or data_im is None:
                raise RuntimeError("FID_real and FID_imag are expected to be present for version 2")
            if data is not None:
                raise RuntimeError("FID is not expected to be present for version 2")
            data = data_re - data_im * 1j
        else:
            raise RuntimeError(f"Unknown file format version {ver}")

        time_dt = time[1:] - time[:-1]
        # Check that all time_dt values are equal (within floating point tolerance)
        if not np.allclose(time_dt, time_dt[0], atol=1e-8, rtol=0):
            raise RuntimeError("All time deltas in 'time' must be equal")
        time_dt = time_dt[0]
        if np.any(np.abs(time_dt - dt) > 1e-6):
            raise RuntimeError(f"dt vs sample frequency mismatch: {time_dt} != {dt}")

        if len(data) != sample_len:
            raise RuntimeError(f"Sample length mismatch: {len(data)} != {sample_len}")
        ...
        if pulse_sequence == "megapress":
            if "EDIT_OFF" in file:
                ...
                raise RuntimeError(
                    "Loaded csv file for MEGA-PRESS, but acquisition cannot be determined.\n"
                    'Add "EDIT_OFF", "EDIT_ON" or "DIFF" into the filepath anywhere.'
                )
        else:
            raise RuntimeError(f"Pulse sequence {pulse_sequence} not supported")
    ...
    def estimate_linewidth(self, method="water_peak", verbose=0, min_snr=3.0, max_peaks=3):
        ...
        if method == "water_peak":
            return self._estimate_linewidth_water_peak_robust(
                magnitude, nu, verbose, min_snr, max_peaks
            )
        elif method == "metabolite_peak":
            return self._estimate_linewidth_metabolite_peak_robust(
                magnitude, nu, verbose, min_snr, max_peaks
            )
            ...
            if lw is None:
                lw = self._estimate_linewidth_water_peak_robust(
                    magnitude, nu, verbose, min_snr, max_peaks
                )
            if lw is None:
                lw = self._estimate_linewidth_metabolite_peak_robust(
                    magnitude, nu, verbose, min_snr, max_peaks
                )
            return lw
        else:
    ...
    def _estimate_linewidth_metabolite_peak(self, magnitude, nu, verbose):
        ...
        if verbose > 2:
            print(
                f"# Strongest metabolite peak found at {peak_ppm:.2f} ppm with magnitude {peak_magnitude:.2e}"
            )
    ...
    def _estimate_linewidth_water_peak_robust(
        self, magnitude, nu, verbose, min_snr=3.0, max_peaks=3
    ):
        ...
        peaks = self._find_peaks_robust(
            water_magnitude, water_nu, noise_level, min_snr, max_peaks, verbose
        )
        ...
        for peak_ppm, peak_magnitude, peak_idx in peaks:
            fwhm_hz = self._calculate_fwhm_interpolated(
                water_magnitude, water_nu, peak_idx, peak_magnitude, verbose
            )
    ...
    def _estimate_linewidth_metabolite_peak_robust(
        self, magnitude, nu, verbose, min_snr=3.0, max_peaks=3
    ):
        ...
        # Find multiple peaks above SNR threshold
        peaks = self._find_peaks_robust(
            metabolite_magnitude, metabolite_nu, noise_level, min_snr, max_peaks, verbose
        )
        ...
        for peak_ppm, peak_magnitude, peak_idx in peaks:
            fwhm_hz = self._calculate_fwhm_interpolated(
                metabolite_magnitude, metabolite_nu, peak_idx, peak_magnitude, verbose
            )
            if fwhm_hz is not None:
                fwhm_values.append(fwhm_hz)
                if verbose > 1:
    ...
    def _find_peaks_robust(self, magnitude, nu, noise_level, min_snr, max_peaks, verbose):
        ...
        for i in sort_indices[:max_peaks]:
            ...
            if snr >= min_snr:
                ...
                if verbose > 2:
                    print(
                        f"# Peak at {peak_ppm:.2f} ppm: SNR={snr:.1f}, magnitude={peak_magnitude:.2e}"
                    )
    ...
    def _calculate_fwhm_interpolated(self, magnitude, nu, peak_idx, peak_magnitude, verbose):
        ...
        left_ppm = self._find_half_max_interpolated(
            magnitude, nu, peak_idx, half_max, direction="left"
        )
        right_ppm = self._find_half_max_interpolated(
            magnitude, nu, peak_idx, half_max, direction="right"
        )
    ...
    def _estimate_linewidth_lorentzian(self, magnitude, nu, verbose, min_snr=3.0, max_peaks=3):
        ...
        for peak_ppm, peak_magnitude, _ in peaks:
            try:
                ...
                region_mask = (metabolite_nu >= peak_ppm - region_width) & (
                    metabolite_nu <= peak_ppm + region_width
                )
                ...
                r_squared = 1 - np.sum((fit_magnitude - fitted_curve) ** 2) / np.sum(
                    (fit_magnitude - np.mean(fit_magnitude)) ** 2
                )

                if verbose > 2:
                    print(
                        f"# Lorentzian fit details at {peak_ppm:.2f} ppm: R²={r_squared:.3f}, linewidth={fitted_linewidth_ppm:.3f} ppm, amplitude={popt[0]:.1f}"
                    )
                ...
                if r_squared > 0.5 and 0.01 < fitted_linewidth_ppm < 2.0:  # More lenient criteria
                    ...
                    if verbose > 1:
                        print(
                            f"# Lorentzian fit at {peak_ppm:.2f} ppm: {fitted_linewidth_hz:.1f} Hz (R²={r_squared:.3f})"
                        )
                else:
                    if verbose > 2:
                        print(
                            f"# Lorentzian fit at {peak_ppm:.2f} ppm rejected: R²={r_squared:.3f}, linewidth={fitted_linewidth_ppm:.3f} ppm"
                        )
        ...
        if verbose > 2:
            print(
                f"# Lorentzian linewidth: {median_linewidth:.1f} Hz (from {len(linewidth_values)} peaks)"
            )
