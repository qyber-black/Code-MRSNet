...
def fida_spectra(metabolite_names, omega, linewidth=1.0, npts=4096, adc_dt=4e-4, save_dir=os.path.join('data', 'basis', 'fida')):
    ...
    try:
        ...
        if e.errno == errno.ENOENT:
            raise Exception('Matlab is not installed on this system! Can\'t simulate FID-A spectra.\n'
                            'You can simulate them on another system, and put them into ' + os.path.join('data', 'basis', 'fida')) from e
        else:
            raise
    p.wait()
